...
# Custom script to load MNIST data set, you can pass imgSize = [11,11] and Quant=4 to get mnist 11x11
...
for k in range(3):
    model = tf.keras.Sequential([
        ...
        layers.FC_AConnect(128, Wstd=Wstd[k], Bstd=Bstd[k], isBin="no", pool=None, Slice=1,
                           d_type=tf.dtypes.float32, weights_regularizer=None, bias_regularizer=None),
        # tf.keras.layers.Dense(10),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.ReLU(),
        layers.FC_AConnect(10, Wstd=Wstd[k], Bstd=Bstd[k], isBin="no", pool=None, Slice=1,
                           d_type=tf.dtypes.float32, weights_regularizer=None, bias_regularizer=None),
    ...
    model.compile(optimizer=optimizer, loss=[
        'sparse_categorical_crossentropy'], metrics=['accuracy', top5])
    history = model.fit(x_train, y_train, validation_split=0.2,
                        epochs=20, batch_size=256)
    loss, accuracy, top5acc = model.evaluate(x_test, y_test)
    model.save("MNIST_test_"+wstd+".h5", include_optimizer=True)
...
a = a.flatten()
b = np.loadtxt('MNIST28X28_8Bits_'+'30'+'_simerr_30_30.txt')
b = b.flatten()
c = np.loadtxt('MNIST28X28_8Bits_'+'30'+'_simerr_50_50.txt')
c = c.flatten()
d = np.loadtxt('MNIST28X28_8Bits_'+'30'+'_simerr_70_70.txt')
d = d.flatten()
data1 = [a,b,c,d]
a = np.loadtxt('MNIST28X28_8Bits_'+'50'+'_simerr_0_0.txt')
a = a.flatten()
b = np.loadtxt('MNIST28X28_8Bits_'+'50'+'_simerr_30_30.txt')
b = b.flatten()
c = np.loadtxt('MNIST28X28_8Bits_'+'50'+'_simerr_50_50.txt')
c = c.flatten()
d = np.loadtxt('MNIST28X28_8Bits_'+'50'+'_simerr_70_70.txt')
d = d.flatten()
data2 = [a,b,c,d]
a = np.loadtxt('MNIST28X28_8Bits_'+'70'+'_simerr_0_0.txt')
a = a.flatten()
b = np.loadtxt('MNIST28X28_8Bits_'+'70'+'_simerr_30_30.txt')
b = b.flatten()
c = np.loadtxt('MNIST28X28_8Bits_'+'70'+'_simerr_50_50.txt')
c = c.flatten()
d = np.loadtxt('MNIST28X28_8Bits_'+'70'+'_simerr_70_70.txt')
d = d.flatten()
data3 = [a,b,c,d]
x = [data1,data2,data3]
blue_line = [0, 0, 1]
